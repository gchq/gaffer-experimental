...
describe('Get Graph By Id Repo', () => {
    ...
    it('should throw RestApiError with title and detail from response body', async () => {
        ...
        await expect(repo.get('notfound-graph')).rejects.toEqual(new RestApiError('Server Error', 'Something went wrong'));
    });
    ...
    it('should throw Unknown RestApiError when status and response body is undefined', async () => {
        ...
        await expect(repo.get('notfound-graph')).rejects.toEqual(new RestApiError('Unknown Error', 'Unable to make request'));
    });
});

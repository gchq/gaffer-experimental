...
describe('Create Graph Repo', () => {
    describe('On Success', () => {
        it('should request an ACCUMULO store graph when ACCUMULO is paramete', async () => {
            const request: ICreateSimpleGraphRequestBody = { graphId: 'accumulo-graph', description: 'a description', storeType: 'ACCUMULO' };
            mock.onPost('/graphs', request).reply(201);

            await expect(repo.create('accumulo-graph', 'a description', StoreType.ACCUMULO)).resolves.toEqual(undefined);
        });

        it('should request an ACCUMULO store graph when MAPSTORE is paramete', async () => {
            const request: ICreateSimpleGraphRequestBody = { graphId: 'map-graph', description: 'a description', storeType: 'MAPSTORE' };
            mock.onPost('/graphs', request).reply(201);

            await expect(repo.create('map-graph', 'a description', StoreType.MAPSTORE)).resolves.toEqual(undefined);
        });
    });

    describe('On Error', () => {
        it('should throw RestApiError 400 message when API no response body', async () => {
            const request: ICreateSimpleGraphRequestBody = { graphId: 'bad-request-graph',description: 'a description', storeType: 'MAPSTORE' };
        ...
        it('should throw RestApiError with title and detail from error response body', async () => {
            const request: ICreateSimpleGraphRequestBody = { graphId: 'forbidden-graph',description: 'a description', storeType: 'MAPSTORE' };

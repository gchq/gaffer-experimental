...
describe("Create Graph Repo", () => {
  describe("On Success", () => {
    it("should request an ACCUMULO store graph when ACCUMULO is paramete", async () => {
      ...
      await expect(
        repo.create("accumulo-graph", "a description", StoreType.ACCUMULO, "", "")
    ...
    it("should request an ACCUMULO store graph when MAPSTORE is paramete", async () => {
      ...
      await expect(
        repo.create("map-graph", "a description", StoreType.MAPSTORE, "", "")
      ).resolves.toEqual(undefined);
    });
  });
  ...
  describe("On Error", () => {
    it("should throw RestApiError 400 message when API no response body", async () => {
      ...
      await expect(
        repo.create("bad-request-graph", "a description", StoreType.MAPSTORE, "", "")
    ...
    it("should throw RestApiError with title and detail from error response body", async () => {
      ...
      mock
        .onPost("/graphs", request)
        .reply(403, { title: "Forbidden", detail: "Kubernetes access denied" });

      await expect(
        repo.create("forbidden-graph", "a description", StoreType.MAPSTORE, "", "")
      ).rejects.toEqual(

import { Box, Button, Container, CssBaseline, Dialog, DialogContent, Grid, IconButton, makeStyles, Slide, Toolbar, Tooltip, Typography, Zoom } from "@material-ui/core"
...
const Transition = React.forwardRef((props: TransitionProps & { children?: React.ReactElement<any, any> }) => <Slide direction="up" {...props} />)
...
export default class CreateGraph extends React.Component<{}, IState> {
  ...
  private disableSubmitButton(): boolean {
    ...
    return (!this.currentStoreTypeIsFederated() && (!elements || !types)) || !graphId || !description || (this.currentStoreTypeIsFederated() && selectedGraphs.length === 0) || (!this.currentStoreTypeIsFederated() && !new ElementsSchema(elements).validate().isEmpty()) || (!this.currentStoreTypeIsFederated() && !new TypesSchema(types).validate().isEmpty())
  }
            ...
            <div className={this.classes.paper}>
              ...
              <form className={this.classes.form} noValidate>
                <Grid container spacing={2}>
                  <GraphIdDescriptionInput graphIdValue={this.state.graphId} onChangeGraphId={(graphId) => this.setState({ graphId })} descriptionValue={this.state.description} onChangeDescription={(description) => this.setState({ description })} />
                  ...
                  {federatedStoreIsNotSelected() && (
                    <>
                      <Grid item xs={12} container direction="row" justify="flex-end" alignItems="center" id={"test"}>
                        ...
                        <Tooltip TransitionComponent={Zoom} title="Add Empty Elements and Types Schema Templates">
                          <IconButton
                            onClick={() => {
                              this.setState({
                        ...
                        <Tooltip TransitionComponent={Zoom} title="Add Elements and Types Schemas From File">
                        ...
                        <Dialog id="dropzone" open={this.state.dialogIsOpen} TransitionComponent={Transition} keepMounted onClose={closeDialogBox} style={{ minWidth: "500px" }} aria-labelledby="alert-dialog-slide-title" aria-describedby="alert-dialog-slide-description">
                  ...
                  <SchemaInput hide={!federatedStoreIsNotSelected()} elementsValue={this.state.elements} onChangeElementsSchema={(elements) => this.setState({ elements })} typesSchemaValue={this.state.types} onChangeTypesSchema={(types) => this.setState({ types })} />
                  <Grid item xs={12} container direction="row" justify="flex-end" alignItems="center" />
                </Grid>
              </form>
              <AddProxyGraphInput
              ...
              />
              <ProxyGraphsTable hide={federatedStoreIsNotSelected()} graphs={this.state.graphs} selectedGraphs={this.state.selectedGraphs} onClickCheckbox={(selectedGraphs) => this.setState({ selectedGraphs })} />
            </div>
          </Container>

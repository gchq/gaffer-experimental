  ...
  function addEdgeReducer(draft: any, action: any) {
    switch (action.type) {
      ...
      case "validateEdgeSource":
        draft.edgeSource.hassErrors = false
        draft.edgeSource.value = action.value
        if (draft.edgeSource.value.length === 0) {
          draft.edgeSource.hassErrors = true
        }
        return

      case "validateEdgeDestnation":
        draft.edgeDestnation.hassErrors = false
        draft.edgeDestnation.value = action.value
        if (draft.edgeDestnation.value.length === 0) {
          draft.edgeDestnation.hassErrors = true
        }
        return

      case "validateEntityDirected":
        draft.entityDirected.hassErrors = false
        draft.entityDirected.value = action.value
        if (draft.entityDirected.value.length === 0) {
          draft.entityDirected.hassErrors = true
        }
        return
    }
  }
